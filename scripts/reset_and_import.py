...
def import_labeled_data(labeled_data_path: Path, config: AppConfig) -> None:
    ...
    result = subprocess.run([
        sys.executable, "scripts/import_labeled_data.py",
        "--data-path", str(labeled_data_path)
...
def main():
    ...
    parser.add_argument(
        "--train-model",
        action="store_true",
        help="Train ML model after importing data"
    )
    parser.add_argument(
        "--use-sample-data",
        action="store_true", 
        help="Use sample test data instead of real labeled data"
    )
    parser.add_argument(
        "--dev-mode",
        action="store_true",
        help="Use development database instead of production database"
    )
