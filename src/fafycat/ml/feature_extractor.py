...
class TextPreprocessor:
    ...
    def __init__(self):
        self.stopwords = {
            'und', 'oder', 'der', 'die', 'das', 'ein', 'eine', 'einen',
            'vom', 'zum', 'zur', 'am', 'im', 'an', 'auf', 'bei', 'mit'
        }
...
class FeatureExtractor:
    ...
    def extract_features(self, transaction: TransactionInput) -> dict[str, Any]:
        ...
        features = {
            ...
            'is_online': int(any(x in transaction.purpose.lower()
                               for x in ['online', 'internet', 'paypal', 'amazon'])),
            'is_recurring': int(any(x in transaction.purpose.lower()
                                   for x in ['dauerauftrag', 'standing order', 'subscription'])),

            # Merchant category indicators
            'is_supermarket': int(any(x in clean_merchant.lower()
                                     for x in ['edeka', 'rewe', 'aldi', 'lidl', 'kaufland', 'netto'])),
            'is_gas_station': int(any(x in clean_merchant.lower()
                                     for x in ['shell', 'esso', 'aral', 'bp', 'total', 'tankstelle'])),
            'is_restaurant': int(any(x in clean_merchant.lower()
                                    for x in ['mcdonald', 'burger', 'pizza', 'restaurant', 'cafe'])),
            'is_transport': int(any(x in clean_merchant.lower()
                                   for x in ['deutsche bahn', 'db ', 'bvg', 'uber', 'taxi'])),
            'is_tech': int(any(x in clean_merchant.lower()
                              for x in ['amazon', 'apple', 'google', 'microsoft', 'netflix'])),

            # Text for TF-IDF
            'text_combined': self.text_preprocessor.process(
                f"{transaction.name} {transaction.purpose}"
            ),
    ...
    def get_numerical_feature_names(self) -> list[str]:
        ...
        return [
            'amount', 'amount_abs', 'amount_log', 'is_income', 'is_round_amount',
            'amount_magnitude', 'day_of_month', 'day_of_week', 'month',
            'is_weekend', 'is_month_start', 'is_month_end', 'is_holiday_season',
            'merchant_length', 'merchant_word_count', 'is_lastschrift',
            'is_dauerauftrag', 'is_kartenzahlung', 'is_online', 'is_recurring',
            'is_supermarket', 'is_gas_station', 'is_restaurant', 'is_transport',
            'is_tech', 'is_eur'
        ]
