"""Prediction Pipeline: one owner of prediction and Review Priority.

Takes stored transactions through prediction (via a passed-in Categorizer),
Review Priority bucketing against the Auto-approve Threshold, and persists
the outcome. Entry points commit before returning.
...
class ConfidenceCategorizer(Protocol):
    ...
    model_id: str
    """Identity of the model behind the predictions, recorded in the Audit Trail."""
...
def predict_unpredicted(
    ...
    *,
    ...
    return _apply_predictions(db, txns, categorizer, threshold=threshold, trigger=trigger), remaining
...
def predict_new(
    ...
    *,
    ...
    return _apply_predictions(db, txns, categorizer, threshold=threshold, trigger=trigger)
...
def _apply_predictions(
    ...
    *,
    ...
    """Predict, bucket, persist with Audit Trail events, and commit."""
    if not txns:
    ...
    for txn, prediction in zip(txns, predictions, strict=True):
        ...
        if priority is ReviewPriority.AUTO_ACCEPTED:
            record_review_event(
                db,
                txn,
                actor=ReviewActor.AUTO_ACCEPT,
                from_category_id=cast(int | None, previous_category_id),
                to_category_id=prediction.predicted_category_id,
            )
