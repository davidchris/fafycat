...
def test_client_with_ml(test_db, mock_categorizer):
    ...
    with patch("api.upload.get_categorizer", return_value=mock_categorizer):
...
def test_client_no_ml(test_db):
    ...
    with patch("api.upload.get_categorizer", side_effect=mock_get_categorizer_fail):
        from api.dependencies import get_db_session
        app.dependency_overrides[get_db_session] = get_test_db
...
def test_upload_with_ml_predictions(test_client_with_ml, test_db):
    ...
    response = test_client_with_ml.post(
        "/api/upload/csv",
    ...
    restaurant_txn = test_db.query(TransactionORM).filter(
        TransactionORM.name.contains("Restaurant")
    ).first()
    assert restaurant_txn.predicted_category_id == 3  # Dining
    assert restaurant_txn.confidence_score == 0.9
    
    supermarket_txn = test_db.query(TransactionORM).filter(
        TransactionORM.name.contains("Supermarket")
    ).first()
...
def test_upload_without_ml_model(test_client_no_ml, test_db):
    ...
    response = test_client_no_ml.post(
        "/api/upload/csv",
        files={"file": ("test.csv", csv_file, "text/csv")}
    )
...
def test_upload_duplicate_transactions(test_client_with_ml, test_db):
    ...
    response1 = test_client_with_ml.post(
        "/api/upload/csv",
    ...
    response2 = test_client_with_ml.post(
        "/api/upload/csv",
